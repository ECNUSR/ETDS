...
def cat():
    ...
    out = layers.Concatenate()([out, input])
...
def main():
    ...
    os.makedirs('experiments/test_times/ops/cat', exist_ok=True)
    os.makedirs('experiments/test_times/ops/cat/int8', exist_ok=True)

    # to tflite
    model.save('/tmp/cat/cat', overwrite=True, include_optimizer=True, save_format='tf')
    convert_model('/tmp/cat/cat', 'experiments/test_times/ops/cat/cat.tflite', 2, input_channel=3, time=True)

    # to int8 tflite
    convert_model('/tmp/cat/cat', 'experiments/test_times/ops/cat/int8/cat.tflite', 2, input_channel=3, mode='int8', time=True)
