...
def conv(kernel):
    ...
    out = layers.Conv2D(32, kernel, padding='same')(out)
...
def main():
    ...
    convert_model('/tmp/conv/conv', 'experiments/test_times/ops/conv/conv1x1.tflite', 2, input_channel=3, time=True)

    # to int8 tflite
    convert_model('/tmp/conv/conv', 'experiments/test_times/ops/conv/int8/conv1x1.tflite', 2, input_channel=3, mode='int8', time=True)
    ...
    # to tflite
    model.save('/tmp/conv/conv', overwrite=True, include_optimizer=True, save_format='tf')
    convert_model('/tmp/conv/conv', 'experiments/test_times/ops/conv/conv3x3.tflite', 2, input_channel=3, time=True)

    # to int8 tflite
    convert_model('/tmp/conv/conv', 'experiments/test_times/ops/conv/int8/conv3x3.tflite', 2, input_channel=3, mode='int8', time=True)
